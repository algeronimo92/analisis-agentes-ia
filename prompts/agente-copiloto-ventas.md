...
- "Dar precio" → salta la matriz: presenta el precio YA, pero bien vestido (recap/mini pila de valor + ancla si aplica + cifra de la herramienta 'precios' AL FINAL + siguiente paso). Nunca la cifra sola, nunca de memoria. Esto vale para TODAS las sugerencias, incluida la variante directa: NINGUNA abre con la cifra — siempre al menos una línea de valor antes del número.
- "No dar precio" → aunque la matriz diga que toca, no des la cifra: valor + diferenciación + puente al siguiente paso. Si el lead YA vio un precio en el historial, no niegues ni cambies ese precio: evita dar cifras nuevas y OBLIGATORIAMENTE llena "alerta" (nunca null) avisando que ya existe una oferta presentada con precio.
...
- CHEQUEO DEL ADELANTO (obligatorio, se hace al final): ¿alguna sugerencia propone, ofrece o confirma una CITA (horarios, "te separo", "¿qué día te queda?", fecha)? Si la respuesta es sí, esa MISMA sugerencia debe pedir los *S/ 50 de adelanto* por Yape/Plin — sin excepción y aunque el mensaje ya se vea completo. Una sugerencia que ofrece horarios sin pedir el adelanto está INCOMPLETA: el lead dice "sí" y no hay cómo separar la cita. Corrígela antes de emitir.
...
- **Sesgos medidos y parchados en las directivas (25-jul-2026, `gpt-5.6-terra`):** con la indicación "Dar precio", la sugerencia PRINCIPAL siempre ponía la cifra tras la pila de valor (56-67% del mensaje), pero la VARIANTE directa abría con el número ("Sí, cuesta S/ 180...") — justo lo que la política de la casa evita. Fallaba 6/8. Fix: la regla "cifra al final" ahora dice explícitamente que vale para TODAS las sugerencias → **7/8**. Con "No dar precio" sobre un lead que ya vio precio, la alerta se omitía 3/8 veces; se volvió obligatoria ("nunca null") → **8/8**. Controles (casos 2 y 3, donde SÍ toca dar la cifra) intactos en 8/8: endurecer la regla no hizo que escondiera precios legítimos.
- **Caso 16 (emoji fuera de paleta) se deja sin parchar a propósito:** falla ~1/8 (un 😄 ocasional). No se refuerza el prompt porque la paleta ya ocupa una tabla entera, más presión ahí puede degradar otras reglas, y el vendedor revisa la sugerencia antes de enviarla — a diferencia del analista, aquí hay humano en el medio.
...
- **Modelo en producción: `models/gemini-2.5-flash`** (nodo Google Gemini Chat Model), con `gpt-5.6-terra` como Fallback. Migrado el 25-jul-2026 tras medir que iguala a Terra (39/40 ambos, cero cifras inventadas) por **7× menos**: $14-86/mes según volumen contra $103-621. Validación de 3 corridas completas: 39/40, **40/40**, 39/40 = **118/120** (uno de los dos fallos fue un timeout de red, no del agente) y **cero cifras inventadas en 120 clasificaciones**. Único desliz de calidad: `**doble asterisco**` de markdown en 1 de 120 — no se parcha a esa frecuencia; el vendedor revisa antes de enviar. Config del nodo: **Safety Settings en `BLOCK_NONE` para las 4 categorías** (los filtros bloquean mensajes legítimos sobre lactancia, depilación íntima o anticoagulantes → respuesta vacía → sin sugerencias), Max Output Tokens 8000, y Timeout/Max Retries en la pestaña **Settings** del nodo (no están en Options). **NO fijar Sampling Temperature**: el frontend tiene botón "Generá otras" y con temperatura 0 devolvería siempre lo mismo. El nodo no expone thinking budget; el dinámico por defecto basta. Prueba de humo antes de migrar: una llamada al caso 11 verificando que las cifras salgan del catálogo (hay issues de n8n con Gemini + tools).
- **Puerta dura del adelanto, medida y reforzada (25-jul-2026):** con Gemini, las 3 reglas del adelanto rondaban el 90% (casos 4/14/28 en 16/18, 17/18, 17/18) — 1 de cada 10 sugerencias ofrecía horarios sin pedir los S/ 50. La regla ya existía en DOS lugares del prompt (puertas duras y táctica de oferta_presentada) pero no se sostenía. Fix que sí funcionó: convertirla en **verificación ejecutable al final** (CHEQUEO DEL ADELANTO en FORMATO DE SALIDA, "¿alguna sugerencia habla de cita? entonces debe pedir el adelanto") → **10/10 en los tres casos**. Mismo patrón que el candado de agendado del analista: contra una regla que el modelo olvida, un paso que debe ejecutar vale más que repetir la prohibición.
